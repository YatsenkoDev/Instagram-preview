...
class WebViewBloc {
  ...
  final _authUrl = 'https://api.instagram.com/oauth/authorize?client_id=${InstaKeys.appId}&redirect_uri=${InstaKeys.redirectUrl}&scope=user_profile,user_media&response_type=code';
  final _codeEnding = '#_';
  ...
  WebViewBloc(BuildContext context) {
    _flutterWebViewPlugin.onUrlChanged.listen((url) async {
      ...
      if (url.contains(_authFinishedUrl)) {
        String code = url.split(_authFinishedUrl)?.last?.split(_codeEnding)?.first;
